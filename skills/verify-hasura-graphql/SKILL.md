...
### 검증 항목
- **V15-HASURA-FOUND**: 프로젝트에 Hasura 설정이 감지됨 (hasura/, docker-compose.yaml에서 hasura 이미지)
- **V15-RAW-SQL-FORBIDDEN**: Hasura 프로젝트에서 raw SQL 사용 금지 (sql.DB 사용, .Query*, .Exec* 호출)
- **V15-MISSING-GRAPHQL**: GraphQL 클라이언트가 서비스에서 사용되지 않음 (gqlClient 필드 누락)
- **V15-SQL-IMPORT**: database/sql 패키지 import 감지 (GraphQL로 대체해야 함)
